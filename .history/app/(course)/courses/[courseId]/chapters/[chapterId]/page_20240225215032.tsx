...
const ChapterPage = async ({
  ...
  const purchase = await Purchase.findOne({userId: userId, courseId: courseId})
  
  const userProgress =await UserProgress.find({userId: userId, chapterId: chapterId})
  
  const nextChapter = JSON.parse(JSON.stringify(await Chapter.findOne({
    courseId: courseId, 
    isPublished: true,
    position: {
    $gt: chapter?.position,
  ...
  if (!!purchase) {
    attachments = JSON.parse(JSON.stringify(await Attachment.find({courseId: courseId})))
  }
        ...
        <div>
          ...
          {!attachments.length && (
            <>
              <Separator />
              <div className="p-4">
                {attachments.map((attachment: {_id: string, url:string, name: string}) => (
                  <a 
                    href={attachment.url}
                    target="_blank"
                    key={attachment._id}
