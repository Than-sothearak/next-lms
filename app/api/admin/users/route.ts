...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: error?.errors?.[0]?.longMessage || "Unable to create Clerk user",
      },
      { status: 400 },
    );
  }
...
export async function GET(request: Request) {
  ...
  const fetchAllUsers = async () => {
    ...
    for (
      let pageOffset = firstPage.data.length;
      pageOffset < firstPage.totalCount;
      pageOffset += 500
    ) {
  ...
  const filteredUsers = sourceUsers.filter((user) => {
    ...
    const matchesYear =
      !yearFilter ||
      new Date(user.createdAt).getFullYear() === Number(yearFilter);
    return matchesRole && matchesYear;
  });
  const years = Array.from(
    new Set(
      allUsers
        .map((user) => new Date(user.createdAt).getFullYear())
        .filter(Number.isFinite),
    ),
  ).sort((a, b) => b - a);
  const pageUsers = filteredUsers.slice(offset, offset + 10);
  await mongooseConnect();
  const allPublicCourses = await Course.find(
    { isPublished: true },
    { _id: 1, title: 1, categoryId: 1 },
  ).lean();
  const courses = await Course.find(
    { isPublished: true, ...(categoryId ? { categoryId } : {}) },
    { _id: 1, title: 1, categoryId: 1 },
  ).lean();
  const categories = await Category.find({}, { _id: 1, name: 1 }).lean();
  const publishedLessonIds = await Chapter.distinct("_id", {
    courseId: { $in: allPublicCourses.map((course) => course._id) },
    isPublished: true,
  });
  ...
  const results = await Promise.all(
    pageUsers.map(async (user) => {
      const [allCourseProgress, sessionList] = await Promise.all([
        Promise.all(
          allPublicCourses.map(async (course) => {
            const courseChapterIds = await Chapter.distinct("_id", {
              courseId: course._id,
              isPublished: true,
            });
            const courseCompletedIds = await UserProgress.distinct(
              "chapterId",
              {
                userId: user.id,
                isCompleted: true,
            ...
            const category = categories.find(
              (item) => String(item._id) === String(course.categoryId),
            );
            return {
              id: String(course._id),
              title: course.title,
              category: category?.name || "Uncategorized",
              progress: courseChapterIds.length
                ? Math.min(
                    100,
                    Math.round(
                      (courseCompletedIds.length / courseChapterIds.length) *
                        100,
                    ),
                  )
                : 0,
        ...
        client.sessions
          .getSessionList({ userId: user.id, limit: 1 })
          .catch((error) => {
            console.error(
              `[ADMIN_USER_SESSIONS] Unable to load sessions for ${user.id}`,
              error,
            );
            return null;
          }),
      ...
      const completedCourses = allCourseProgress.filter(
        (course) => course.progress === 100,
      ).length;
      const completedLessonIds = await UserProgress.distinct("chapterId", {
        userId: user.id,
        isCompleted: true,
        chapterId: { $in: publishedLessonIds },
      });
      const completedLessons = completedLessonIds.length;
      const progress = totalPublishedLessons
        ? Math.round((completedLessons / totalPublishedLessons) * 100)
        : 0;
      const visibleCourseIds = new Set(
        courses.map((course) => String(course._id)),
      );
      const courseProgress = allCourseProgress.filter((course) =>
