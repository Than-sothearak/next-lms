...
export const getChapter = async ({
  ...
  try {
    ...
    nextChapter = await Chapter.find({courseId: courseId, isPublished: true}).sort({position: -1})

    const userProgress = await UserProgress({userId: userId, chapterId: chapterId})
    return {
      chapter,
      course,
      muxData,
